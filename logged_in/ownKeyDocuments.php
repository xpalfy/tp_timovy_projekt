...
<head>
    ...
    <style>
        ...
        .text-papyrus { color: #3b2f1d; }
        .card-img { max-height: 100%; object-fit: cover; }
        .card-pic { @apply bg-white rounded-xl shadow-md p-4 transition-transform transform hover:scale-105; }
        .display-btns { display: flex; justify-content: center; gap: 10px; margin-bottom: 20px; }
        .item-display-btn, .list-display-btn {
        ...
        .item-display-btn:hover, .list-display-btn:hover { background-color: #cdbf9b; transform: scale(1.10); }
        .item-display-btn.active, .list-display-btn.active { background-color: #cdbf9b; font-weight: bold; }
        .item-display-btn { background-image: url('../img/item.png?'); background-size: 20px 20px; background-repeat: no-repeat; background-position: center left 10px; }
        .list-display-btn { background-image: url('../img/list.png?'); background-size: 20px 20px; background-repeat: no-repeat; background-position: center left 10px; }
</style>
...
<main class="flex-grow">
    ...
    <div class="w-full max-w-md">
        <label for="search-input" class="block mb-2 text-lg font-medium text-[#3b2f1d]">🔎 Search documents</label>
        <input 
            type="text" 
            id="search-input" 
    ...
    <!-- Select Field -->
    <div class="w-full max-w-md">
        <label for="filter-select" class="block mb-2 text-lg font-medium text-[#3b2f1d]">📂 Filter by type</label>
        <select 
            id="filter-select" 
...
function fetchSharedDocumentsAndImages() {
    Promise.all([
        fetch('fetchSharedDocuments.php?key=KEY').then(res => res.json()),
        fetch('fetchSharedImages.php?key=KEY').then(res => res.json())
    ])
    .then(([docs, imgs]) => {
        documentsData = docs;
        imagesData = imgs;
...
function fetchPublicDocumentsAndImages() {
    Promise.all([
        fetch('fetchDocuments.php?key=KEY&public=true').then(res => res.json()),
        fetch('fetchImages.php?key=KEY&public=true').then(res => res.json())
    ])
    .then(([docs, imgs]) => {
        documentsData = docs;
        imagesData = imgs;
...
function fetchDocumentsAndImages() {
    Promise.all([
        fetch('fetchDocuments.php?key=KEY&public=false').then(res => res.json()),
        fetch('fetchImages.php?key=KEY&public=false').then(res => res.json())
    ])
    .then(([docs, imgs]) => {
        documentsData = docs;
        imagesData = imgs;
